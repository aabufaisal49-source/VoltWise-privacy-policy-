...
export function generateBilingualPolicyHtml(config: AppConfig): string {
  ...
  html += `  <!-- Arabic Version Section -->\n`;
  html += `  <section id="arabic-policy" class="bg-white rounded-md">\n`;
  html += generateArabicPolicyHtml(config);
  html += `  </section>\n\n`;
  html += `  <hr class="border-t-2 border-dashed border-neutral-300 my-8" />\n\n`;
  html += `  <!-- English Version Section -->\n`;
  html += `  <section id="english-policy" class="bg-white rounded-md">\n`;
